...
### Added
- `results.json` schema (MVP): a new `utils/results_parser.py` module defines the contract-pinned per-run results format (`run_id`, `request_timestamp`, `set`, `tier`, `arch`, `source`, `target`, `verdict`, `tests[]`, `total_duration_seconds`; verdict enum `PASSED|FAILED|SKIPPED|ERROR|CANCELED`) plus `write_results_json()`/`parse_results_json()` with strict validation (required fields, verdict enum membership, ISO 8601 timestamps, CANCELED-implies-empty-tests) and byte-identical xunit colocation. New `results_dir()` state path (`~/.local/share/enge/results/`, XDG/config-overridable, mirrors the manifest store). Nothing calls this yet — `enge report` integration (manifest lookup, actual writes) is deferred to a separate feature branch; this entry documents the module/schema landing standalone
